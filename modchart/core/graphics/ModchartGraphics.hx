...
class ModchartHoldRenderer extends ModchartRenderer<FlxSprite>
{
    ...
    override public function prepare(item:FlxSprite):Void
    {
        ...
        var basePos = new Vector3D(
            Adapter.instance.getDefaultReceptorX(lane, player),
            Adapter.instance.getDefaultReceptorY(lane, player)
...
class ModchartArrowRenderer extends ModchartRenderer<FlxSprite>
{
    override public function prepare(arrow:FlxSprite)
    {
        ...
        helperVector.setTo(
            Adapter.instance.getDefaultReceptorX(arrowData.receptor, arrowData.field) + Manager.ARROW_SIZEDIV2,
		...
		if (orient != 0)
		{
			final nextOutput = instance.modifiers.getPath(
                new Vector3D(
                    Adapter.instance.getDefaultReceptorX(arrowData.receptor, arrowData.field) + Manager.ARROW_SIZEDIV2,
                    Adapter.instance.getDefaultReceptorY(arrowData.receptor, arrowData.field) + Manager.ARROW_SIZEDIV2
                ),
                arrowData,
                1
            );
