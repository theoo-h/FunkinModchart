...
class ModifiersMacro {
	public static macro function get():ExprOf<Iterable<Class<Dynamic>>> {
		...
		var request:String = 'package~${'modchart.modifiers'}~recursive';

		classListsToGenerate.push(request);

		return macro modchart.core.macros.CompiledClassList.get($v{request});
	}
	...
	static function compileClassLists() {
		var compiledClassList:ClassType = getClassType("modchart.core.macros.CompiledClassList");
